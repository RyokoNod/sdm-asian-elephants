...
write.table(m.frame, file = "Elephas_Maximus/Elephas_Maximus_Predictions_GLM_Y1950_Y2000.txt",
...
for (seed in 1001:1100) {
  ...
  pre.matrix[, seed - 1000] <- predict(m.fit, newdata = terra, type = "prob")[, "1"]
}
...
write.table(m.frame, file = "Elephas_Maximus/Elephas_Maximus_Predictions_RF_R10_Y1950_Y2000.txt",
            quote = FALSE, sep = "\t", row.names = FALSE)
...
for (seed in 2001:2100) {
  ...
  m.fit <- randomForest(PAF ~ ., data = species[, c("PAF", features)], ntree = 100)
  pre.matrix[, seed - 2000] <- predict(m.fit, newdata = terra, type = "prob")[, "1"]
}
...
write.table(m.frame, file = "Elephas_Maximus/Elephas_Maximus_Predictions_RF_S10_Y1950_Y2000.txt",
            quote = FALSE, sep = "\t", row.names = FALSE)
